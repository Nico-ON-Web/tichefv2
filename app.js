...
.then(data=>{
    ...
    data= data.filter(recette=>{
        recette.nom.includes(input.value)
    })
...
fetch("articles-data.json").then(rep=>rep.json()).then(data=>{
    
    data.forEach((article,pos)=>{
        afficheArticle(article,pos)
    })
})

function afficheArticle(art,pos){
    let dataaos = ""
    if(pos %2 == 0){
        dataaos = "fade-right"
    }else{
    ...
    <article data-aos="${dataaos}" class="large-6">
        <div class="flex gap16">
        <div class="large-4">
            <img src="${art.img}" alt="" class="responsive">
        </div>
        <div class="large-8">
            <h3>${art.titre}</h3>
